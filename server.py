...
def add_item(store_id: str, category_key: str, payload: Dict[str, Any]):
    ...
    current_stock = parse_float(payload.get("current_stock"))
    min_stock = parse_float(payload.get("min_stock"))
    ...
    buy_link = (payload.get("buy_link") or "").strip()
...
def update_item(store_id: str, category_key: str, item_id: int, payload: Dict[str, Any]):
    ...
    current_stock = parse_float(payload.get("current_stock", row["current_stock"]))
    min_stock = parse_float(payload.get("min_stock", row["min_stock"]))
    unit = (payload.get("unit", row["unit"]) or "").strip()

    price = (payload.get("price", row["price"]) or "").strip()
    vendor = (payload.get("vendor", row["vendor"]) or "").strip()
    storage = (payload.get("storage", row["storage"]) or "").strip()
    origin = (payload.get("origin", row["origin"]) or "").strip()
    buy_link = (payload.get("buy_link", row["buy_link"]) or "").strip()
    memo = (payload.get("memo", row["memo"]) or "").strip()
...
def delete_item(store_id: str, category_key: str, item_id: int):
    con = db()
    row = con.execute(
        "SELECT 1 FROM items WHERE id=? AND store_id=? AND category_key=?",
        (item_id, store_id, category_key)
    ).fetchone()
    if not row:
        con.close()
