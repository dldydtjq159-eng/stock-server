...
def list_admins(auth=Header(None)):
    return {"admins":[{"id":SUPER_ID,"name":"슈퍼","is_super":True}]+
            [{"id":a["id"],"name":a.get("name",""),"is_super":False} for a in admins["admins"]]}
...
def add_admin(req: AdminAddReq, dec=_auth_super()):
    ...
    admins["admins"].append({"id":req.id,"pw_hash":pwd.hash(req.pw),"name":req.name})
    _save(ADMINS_FILE, admins)
    return {"ok":True}
